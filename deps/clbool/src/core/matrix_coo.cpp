...
namespace clbool {
    ...
    matrix_coo::matrix_coo(Controls &controls,
        ...
        CLB_CREATE_BUF(_rows = utils::create_buffer(controls, nnz), 279791);
        CLB_CREATE_BUF(_cols = utils::create_buffer(controls, nnz), 93270712);
        cl::Event e1, e2;
        ...
        CLB_WRITE_BUF(controls.queue.enqueueWriteBuffer(_cols, CL_FALSE, 0, sizeof(index_type) * nnz, cols_indices,
                                                        nullptr, &e2), 973271);
        CLB_WAIT({e1.wait(); e2.wait();}, 456745642);
    ...
    void matrix_coo::reduce_duplicates(Controls &controls) {
        ...
        cl::Buffer positions;
        CLB_CREATE_BUF(positions = utils::create_buffer(controls, _nnz + 1), 97987319);
        ...
        prepare_positions.run(controls, positions, _rows, _cols, _nnz)), 367279701);
        ...
        cl::Buffer new_rows;
        CLB_CREATE_BUF(new_rows = utils::create_buffer(controls, new_nnz), 9739721);
        cl::Buffer new_cols;
        CLB_CREATE_BUF(new_cols =  utils::create_buffer(controls, new_nnz), 8002871);
        ...
        set_positions.run(controls, new_rows, new_cols, _rows, _cols, positions, _nnz)), 848276172);
    ...
    void matrix_coo::reduce_duplicates2(Controls &controls) {
        ...
        CLB_CREATE_BUF(duplicates_per_tb = utils::create_buffer(controls, groups_num + 1), 87660101);
        ...
        init_duplicates.run(controls, _rows, _cols, duplicates_per_tb, _nnz, groups_num)), 56575111);
        ...
        reduce_tb.run(controls, _rows, _cols, duplicates_per_tb, _nnz)), 98666151);
        ...
        cl::Buffer new_rows;
        CLB_CREATE_BUF(new_rows = utils::create_buffer(controls, new_nnz), 8777621);

        cl::Buffer new_cols;
        CLB_CREATE_BUF(new_cols = utils::create_buffer(controls, new_nnz), 815134914);
        ...
        shift_tb.run(controls, _rows, _cols, new_rows, new_cols, duplicates_per_tb, _nnz)), 102552366);
